...
/*===========================================================================
  ...
  (LJ剛体モデル・コア版 — Serial / OpenMP / OpenACC GPU 統合)
  ...
  機能:
    ...
    - Velocity-Verlet時間積分
    - FCC結晶構造の初期配置を自動生成
    - コンパイル時スイッチで Serial/OpenMP/OpenACC を切替
    - リスタート機能なし、OVITO出力なし (コア版)

  GPU最適化方針:
    1. forces(): gang並列(分子) × vector並列(分子内原子ai)
       - 対称フルリスト使用: Newton第3法則不使用→競合回避
       - ai原子ループをvector化: 最大128スレッドのwarp並列
       - ビリアル9成分をスカラーreduction変数に展開
    2. データ常駐: #pragma acc data で配列をGPU上に保持
    3. OpenMPモード: schedule(dynamic) + atomic でマルチスレッド化
